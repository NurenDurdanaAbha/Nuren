...
int main()
{
    ...
    char a;
    string name,D;
    int num,ID,i=0,j_value,k;
    double BS;

    ///File
    ofstream outFile;
    outFile.open("Employee_informations.txt");
    ...
    if(a=='a')
    {
        ...
        for(int j=0; j<i; j++)
        {
            if(info[i].viewID()==info[j].viewID())
            {
                cout<<endl<<"=> This Employee ID has been used."<<endl<<endl;
                goto ghew;
            }
            else
                break;
    ...
    ///showing in file
         outFile<<endl<<"**Storing Employee info: "<< i+1 <<endl;
         outFile<<endl<<"Employee ID: "<<info[i].viewID()<<endl;
         outFile<<"Employee's Name: "<<info[i].viewName()<<endl;
         outFile<<"Designation: "<<info[i].viewDesignation()<<endl;
         outFile<<"Basic Salary: "<<info[i].viewBS()<<endl;
         outFile<<"Home Bonus: "<<info[i].viewHB()<<endl;
         outFile<<"Medical: "<<info[i].viewMEDICAL()<<endl;
         outFile<<"Total: "<<info[i].viewTOTAL()<<endl;
    ...
    for(int j=0; j<i; j++)
    {
     ...
     ///search result view in file
     if(flag==1)
        outFile<<endl<<endl<<"=> Employee ID: "<< info[k].viewID() << " has been searched"<<endl;
    ...
    for(int j=0; j<i; j++)
    {
     ...
     cout<<"\t\t*****Update list*****"<<endl;
        ...
        if(update==1)
        {
         ...
         string s=info[j_value].viewName();
        char n[10];
        gets(n);
        cout<<"Employee's Name: ";
        getline(cin,name);
        ...
        ///view in file: updated name
            outFile<<endl<<endl<<"=> Updated the name of the Employee ID: "<< info[j_value].viewID() <<" from "<< s <<" to " << info[j_value].viewName()<<endl;

         outFile<<endl<<"- - - Updated info - - -"<<endl;
         outFile<<endl<<"Employee ID: "<<info[j_value].viewID()<<endl;
         outFile<<"Employee's Name: "<<info[j_value].viewName()<<endl;
         outFile<<"Designation: "<<info[j_value].viewDesignation()<<endl;
         outFile<<"Basic Salary: "<<info[j_value].viewBS()<<endl;
         outFile<<"Home Bonus: "<<info[j_value].viewHB()<<endl;
         outFile<<"Medical: "<<info[j_value].viewMEDICAL()<<endl;
         outFile<<"Total: "<<info[j_value].viewTOTAL()<<endl;
        ...
        if(update==2)
        {
         ...
         string s=info[j_value].viewDesignation();
         char n[10];
         gets(n);
         cout<<"Designation: ";
         getline(cin,D);
         ...
         ///view in file: updated Designation
            outFile<<endl<<endl<<"=> Updated the Designation of the Employee ID: "<< info[j_value].viewID() <<" from "<< s <<" to " << info[j_value].viewDesignation()<<endl;
        ...
        ///view in file: updated BS
            outFile<<endl<<endl<<"=> Updated the Basic Salary of the Employee ID: "<< info[j_value].viewID() <<" from "<< a <<" to " << info[j_value].viewBS()<<endl;
